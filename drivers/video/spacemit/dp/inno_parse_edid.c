...
static int Dtd2Vic(unsigned char *i_pTable)
{
	int i = 0;
	int j = 0;
	int s32Vic = 0;
...
static void AudioParse(unsigned char *i_pTable, unsigned char i_u8Off)
{
	int i = 0;
	int j = 0;
...
static void ExtTagParse(unsigned char *i_pTable, unsigned char i_u8Off)
{
	...
	if (u8Tag == CEA_EXT_TAG_CDB) {			// Colorimetry Data Block
		...
		if(u8Tmp & 0x01) osal_printf("\t\t %s\n", "Metadata Profile 0");
		if(u8Tmp & 0x02) osal_printf("\t\t %s\n", "Metadata Profile 1");
		if(u8Tmp & 0x04) osal_printf("\t\t %s\n", "Metadata Profile 2");
		if(u8Tmp & 0x08) osal_printf("\t\t %s\n", "Metadata Profile 3");
