...
static inline __maybe_unused int se_clock_off(struct udevice *dev)
{
    ...
    // disable parent clock would cause APB clock off
	// ret = clk_disable_bulk(&plat->clks);
	// if (ret) {
	// 	pr_err("Failed to disable clk: %d\n", ret);
	// }

    return ret;
}

void efuse_program_poweron(struct udevice *dev)
{
    // uint8_t val;
    // uint8_t i2c_bus = 8;
    ...
    // /* enable ldo15, default 1.8v */
    // printf("enable ldo-15, default voltage is 1.8v ...");
...
void efuse_program_poweroff(struct udevice *dev)
{
//     uint8_t val;
//     uint8_t i2c_bus = 8;
//     uint8_t power_addr = 0x31;
...
int efuse_write_bank_core(struct udevice *dev, uint32_t bank_index, uint32_t *buffer)
{
    ...
    for (select_fuse_b = 0; select_fuse_b < 2; select_fuse_b++) {
        ...
        while (1) {
            value = readl(reg_base + EFUSE_STATUS);
            printf("EFUSE_STATUS(expect FUSE_BURN_DONE) is 0x%x\n", value);
        ...
        while (1) {
            value = readl(reg_base + EFUSE_STATUS);
            printf("EFUSE_STATUS(expect FUSE_READY) is 0x%x\n", value);
            if (value & FUSE_READY)
                break;
        }
        printf("select_fuse_b is 0x%x end\n", select_fuse_b);
    }

    se_clock_off(dev);
    efuse_program_poweroff(dev);
...
{
    int bank_start, bank_end, i, byte_offset, byte_size;
    ...
    for (i = 0; bank_start < bank_end; bank_start++) {
        ...
        byte_size = min(size, (int)sizeof(efuse_data) - byte_offset);
        memcpy(efuse_data, buf + i, byte_size);
        efuse_write_bank_core(dev, bank_start, (uint32_t*)efuse_data);

        efuse_read_bank(dev, bank_start * FUSE_BANK_BYTES,
                        efuse_cmp_data, FUSE_BANK_BYTES);
        if (0 != memcmp(efuse_cmp_data, efuse_cmp_data, FUSE_BANK_BYTES)) {
